...
if ! az sql server show -n $SQL_SERVER -g $RG >/dev/null 2>&1; then
  az sql server create -n $SQL_SERVER -g $RG -l $LOCATION --admin-user ${SQL_ADMIN_USER:-commerceadmin} --admin-password "$EXISTING_PWD" --tags env=$ENV_NAME scale-to-zero=true
...
if ! az sql db show -n $SQL_DB -s $SQL_SERVER -g $RG >/dev/null 2>&1; then
  az sql db create -n $SQL_DB -s $SQL_SERVER -g $RG --edition GeneralPurpose --family Gen5 --capacity 1 --compute-model Serverless --auto-pause-delay 60 --min-capacity 0.5 --max-capacity 2 --backup-storage-redundancy Local --tags env=$ENV_NAME scale-to-zero=true
fi
...
if ! az search service show -n $SEARCH_SERVICE -g $RG >/dev/null 2>&1; then
  az search service create -n $SEARCH_SERVICE -g $RG -l $LOCATION --sku free --tags env=$ENV_NAME
fi
...
echo "" >> "$ROOT/.scale-zero-names.env"
