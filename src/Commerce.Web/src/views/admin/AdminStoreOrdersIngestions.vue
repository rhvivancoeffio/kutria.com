  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    <main class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8">
      <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-6">
        <div class="min-w-0">
          <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Ventas (ingestado)</h1>
          <p class="mt-1 text-sm sm:text-base text-gray-600 dark:text-gray-400">
            Pedidos sincronizados en Table Storage desde la integración seleccionada.
          </p>
        </div>
      ...
      <template v-else>
        ...
        <template v-else-if="selectedIntegrationId">
          ...
          >
            <p class="text-gray-500 dark:text-gray-400">No hay pedidos ingeridos. Usa Sincronizar para cargar datos.</p>
          </div>
...
async function reload(token = null) {
  ...
  error.value = null
  try {
    const data = await apiService.listIngestionsOrders(selectedIntegrationId.value, {
      pageSize,
      nextToken: token
    })
    ...
    error.value = err.response?.data?.error || err.message || 'Error al cargar ventas ingeridas'
    items.value = []
    hasMore.value = false
    nextToken.value = null
  } finally {
...
async function openDetail(row) {
  ...
  try {
    const data = await apiService.getIngestionOrder(selectedIntegrationId.value, row.id)
